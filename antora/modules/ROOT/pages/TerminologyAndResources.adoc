...
=== Foreign Function and Memory API (FFM) Module (built-in)
* OpenCL: 2.2 
* Baseline: Java 26 (preview)
* JVM Options: `--enable-preview --add-modules jdk.incubator.vector`

As generated via *jextract*. 

(e.g. get _jextract_ from https://jdk.java.net/jextract/)

[source,bash]
.Generating OpenCL Java sources (using ROCm 7.14 on Debian)
----
cd /tmp
mkdir generated
jextract \ 
  --output generated \
  --target-package jcompute.opencl.ffm  
  -I/opt/rocm/include \
  -I/opt/rocm/include/CL \
  /opt/rocm/include/CL/opencl.h
----
